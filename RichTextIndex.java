...
//  private final String zkEnsemble = "http://localhost:2181";
  ...
  public static void main(String[] args) {
    try {
    	RichTextIndex idxer = new RichTextIndex("http://localhost:8983/solr");

      idxer.doTikaDocuments(new File("/home/gishas/Diplomski/Apache_solr/full-text/knjige"));
//      idxer.doSqlDocuments();
  ...
  private RichTextIndex(String url) throws IOException, SolrServerException {
    // Create a SolrCloud-aware client to send docs to Solr
    // Use something like HttpSolrClient for stand-alone
	  final List<String> solrUrls = new ArrayList<>();
	  solrUrls.add(url);
	  client = new CloudSolrClient.Builder(solrUrls)
	          .build();
	  client.setDefaultCollection("richText");
